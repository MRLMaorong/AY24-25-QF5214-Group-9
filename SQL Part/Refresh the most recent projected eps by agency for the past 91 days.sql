...
BEGIN
	...
	(
		...
		report_year, forecast_eps, fy, ROW_NUMBER() over(
		partition by stock_code, organ_id, report_year order by create_date desc, entrydate desc) as seq
		from cr1
		where fy = 1
	),
	cr3 as(
		select report_id, stock_code, organ_id, create_date, entrydate, report_year, forecast_eps, fy 
		from cr2
		where
		(case when MONTH(@tradingDay) <= 4 then YEAR(@tradingDay)-1 else YEAR(@tradingDay) end) = report_year
		and seq = 1
	)
	insert into zyyx0524.dbo.eps_latest_report(tradedate,report_id,stock_code, organ_id, create_date, entrydate, report_year, forecast_eps, fy)
	select @tradingDay, report_id, stock_code, organ_id, create_date, entrydate, report_year, forecast_eps, fy
	from cr3
