...
BEGIN
	...
	cr3 as(
		...
		from cr2
		where
		(case when MONTH(@tradingDay) <= 4 then YEAR(@tradingDay) else YEAR(@tradingDay)+1 end) = report_year
		and seq = 1
	)
	insert into zyyx0524.dbo.np_fy2(tradedate,report_id,stock_code, organ_id, create_date, entrydate, report_year, np_fy2, fy)
